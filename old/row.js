class Row {
  constructor(tableContext, data) {
      ...
      this.syncViewData = this.syncViewData.bind(this);
  ...
  createView() {
      ...
      const addBtn = TableView.createButton('+', {
          ...
          onclick: () => {
              let newData = {...this.mapNext(), [this.tableContext.unique]: `[${this.tableContext.unique}]`};
              this.insertNext(new Row(this.tableContext, newData));
          }
      });
  ...
  mapRemove() {
      if (this.next && this.prev) {
          this.tableContext.relations.forEach(({ current, toNext }) => {
              this.next.data[current] = this.prev.data[toNext];
          });
      }
  }
  ...
  mapNext() {
      ...
      if (!this.next) {
          ...
          this.tableContext.relations.forEach(({ current, toNext }) => {
              newData[current] = prevData[toNext];
          });
      } else {
          newData = { ...this.data };
          let prevData = this.data;
          ...
          this.tableContext.relations.forEach(({ current, toNext }) => {
              newData[current] = prevData[toNext];
              newData[toNext] = nextData[current];
          });
      }
      return newData;
  }
  ...
  generateInputAttribute(
      input,
      key,
      notInclude = ['options', 'validation', 'default', 'onchange', 'dataType']
  ) {
      let attr = {};
  ...
  beforeRender() {
      const last = this.tableContext.tail;
      if (this === last) {
          this.tableContext.relations.forEach(({ current, toNext }) => {
